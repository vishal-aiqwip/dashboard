...
type Chatbot = { id: string; system_prompt?: string | null };
...
export function OrgSystemPromptDialog({ open, onOpenChange, organizationId }: Props) {
  const [prompt, setPrompt] = useState('');
  const [isSaving, setIsSaving] = useState(false);

  const { data: chatbots = [], isLoading, isError } = useQuery<Chatbot[]>({
    queryKey: ['chatbots', organizationId, 'with-prompts'],
    queryFn: async () => {
  ...
  const initialState = useMemo(() => {
    ...
    const values = chatbots.map((c) => (c.system_prompt ?? '').trim());
  ...
  const onSave = async () => {
    ...
    try {
      ...
      for (const bot of bots) {
        try {
          await axiosApi.patch(`/api/chatbots/${bot.id}`, { system_prompt: prompt }, { params: { organization_id: organizationId } });
          updated += 1;
        } catch { /* continue */ }
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Set a shared system prompt for all chatbots in this organization. This will overwrite the value on each chatbot.
          </DialogDescription>
          {isLoading ? (
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            placeholder="Enter the system prompt for this org's chatbots…"
          />
        </div>
